...
<table width="100%" cellpadding="0" cellspacing="0"><tr><td width="28%" style="vertical-align:middle;">@if(!empty($uiAssets[$icon]))<img src="{{ $uiAssets[$icon] }}" width="50" height="50" style="width:13.2mm;height:13.2mm;" alt="{{ $label }}">@endif</td><td width="72%" style="vertical-align:middle;"><div class="mini-value-lg">{{ $label }}</div><div class="dashboard-copy" style="margin-top:0.8mm;">{{ $i===0?'Calm reactive redness and support barrier comfort.':($i===1?'Boost moisture and improve skin plumpness.':'Refine visible pores and central-face texture.') }}</div></td></tr></table>
</td>
@endforeach
</tr></table>
...
<table width="100%" cellpadding="0" cellspacing="0" class="card card-purple" style="margin-top:3mm;height:66mm;"><tr><td width="25%" class="pad-sm" style="text-align:center;">@if($imageUrl)<img src="{{ $imageUrl }}" width="155" height="205" style="width:41mm;height:54.2mm;" alt="{{ data_get($item,'parameter_name') }}">@endif</td><td width="55%" class="pad"><div class="parameter-name">{{ data_get($item,'parameter_name') }}</div><div class="parameter-copy">{{ data_get($item,'client_description',data_get($item,'score_explanation','')) }}</div><div class="parameter-causes"><b>Common contributors</b><br>{{ collect(data_get($item,'possible_causes',[]))->take(2)->implode(' | ') }}</div></td><td width="20%" class="pad" style="text-align:center;vertical-align:middle;">@include('pdf.facial.partials.score',['item'=>$item])</td></tr></table>
@endif
