        ...
        stage("Verify SSH connection to server") {
            steps {
                sshagent(credentials: ['jenkins']) {
                    sh '''
                        ssh -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no root@187.127.173.33 whoami
                    '''
                }
            }
        }
    ...
    post {
        success{
            withCredentials([sshUserPrivateKey(credentialsId: "jenkins", keyFileVariable: 'keyfile')]) {
              sh  'rsync -vrzhe "ssh -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa" . root@187.127.173.33:/home/ai-aesthetics-crm/htdocs/crm.ai-aesthetics.in'
            }

            sshagent(credentials: ['jenkins']) {
                sh '''
                    ssh -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no root@187.127.173.33 << EOF
                    whoami
